...
from osrs_gept.models import LatestInfoWithId, LatestPayload, MappingInfo, MappingPayload, SlimMappingInfo, SlimMappingPayload
...
class WikiClient:
    ...
    def get_latest(self, id_: Optional[int] = None) -> List[LatestInfoWithId]:
        ...
        resp = requests.get(f'{self.api_url}/latest', params=params, headers={'User-Agent': self.agent})
        resp.raise_for_status()
        returned_payload = LatestPayload.parse_obj(resp.json())
        return [ LatestInfoWithId(id=id_, **info.dict()) for id_, info in returned_payload.data.items() ]
    
    @cached(WikiObjectsCache, key=lambda self: 'mapping')
    def get_mapping(self) -> MappingPayload:
        resp = requests.get(f'{self.api_url}/mapping', headers={'User-Agent': self.agent})
        resp.raise_for_status()
    ...
    def get_mapping_slim(self) -> SlimMappingPayload:
        ...
        return [SlimMappingInfo(id=item.id_, name=item.name) for item in full_mapping]